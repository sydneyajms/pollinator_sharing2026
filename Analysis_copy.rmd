---
title: "Analysis1"
output: html_document
date: "2026-01-28"
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
library(tidyr)
library(dplyr)
library(bipartite)
library(ggplot2)
library(ggeffects)
library(ggrepel)
library(tidyverse)
library(vegan)
library(ggridges)

```

```{r}
# read in cleaned data
pollinators <- read.csv("Pollinator_data_cleaned.csv")
seeds <- read.csv("Cleaned_seeds.csv")

pollinators$BP <- paste0(pollinators$Block, pollinators$Plot)
seeds$BP <- paste0(seeds$Block, seeds$Plot)
  
# get unique treatments from seed data and name empty list
trts <- unique(seeds$genSpec)

clist <- vector(mode='list', length=length(trts)) #list of communities (treatments)
names(clist) <- trts  #naming the parts of the list the name of the treatment group 

plist <- vector(mode='list', length=15)
names(plist) <- unique(paste(trts, seeds$Block, sep = "_"))  #naming the list 

# create for loop to make each item in list contain block and plots from that treatment
for (tt in trts){
  tmp <- seeds[seeds$genSpec==tt,]
  bp <- unique(tmp$BP)

# create adjacency matrix for each item in list (community) 
  polls <- pollinators[pollinators$BP %in% bp,]
  ppmat <- as.matrix(table(polls$sp, polls$Genus))
  clist[[tt]] <- ppmat
  
for (b in bp){
  select <- paste(tt, substr(b,1,1), sep = "_")
  blockpolls <- polls[polls$BP ==b,]
  ppmatB <- as.matrix(table(blockpolls$sp, blockpolls$Genus))
  plist[[select]] <- ppmatB
}}

#whole experiment network:
pp_all <- as.matrix(table(pollinators$sp, pollinators$Genus))

# find average Open - Closed per plant per group 
zeros <- seeds[,c('OsPerb','CsPerb')]==0 
seeds[,c('OsPerb','CsPerb')][zeros] <- NA 

# find means, differences and ratios of pollinator impact 
seedsGrouped <- seeds %>%
  mutate(ratio_poll_impact = OsPerb/CsPerb) %>%
  group_by(sp, BP) %>%
  mutate(ratio_seedsPerPlant = mean(ratio_poll_impact, na.rm = TRUE)) %>%
 # group_by(sp) %>%
#  mutate(FSM = mean(ratio_seedsPerPlant, na.rm = TRUE)) %>%
  distinct(BP, genSpec, sp, ratio_seedsPerPlant)
  

# add treatment column 
seedsGrouped$treatment <- paste(seedsGrouped$genSpec, substr(seedsGrouped$BP,1,1), sep = "_")

pollsSeedsdf <- seedsGrouped %>%
  inner_join(pollinators, by = c("BP", "sp"), relationship = "many-to-many") %>%
  select(-c(Block, Plot, sp.no)) 

```

Network analysis by block -> group level 

```{r}
# find network and group level characteristics (by block/plot)
H_values <- lapply(plist, networklevel, index = "H2")
nestedness <- lapply(plist, networklevel, index = "nestedness")
niche_overlap <- lapply(plist, grouplevel, level ="both", index = "niche overlap")
Total_niche <- networklevel(pp_all, level ="both", index = "niche overlap")

# convert list of metrics to data frame (by by block/plot)
H_values <- as.data.frame(H_values)
nestedness <- as.data.frame(nestedness)
niche_overlap <- as.data.frame(niche_overlap)
Total_niche <- as.data.frame(Total_niche)

# create function to rename and which overlap is which and rearrange df  
rename.levels <- function(df, metric){
  new_df <- df %>% 
  tibble::rownames_to_column("group")
  new_df$group <- ifelse(grepl("HL", new_df$group), "pollinators", "plants")
  new_df <- pivot_longer(new_df, cols = -group, names_to = "treatment", values_to = metric)
  return(new_df) 
}

# make characteristics into dfs 
nichedf <- rename.levels(niche_overlap, "overlap")
Hdf <- rename.levels(H_values, "H2")
nestednessdf <- nestedness %>%
  tibble::rownames_to_column("metric") %>%
  pivot_longer(cols = -metric, names_to = "treatment", values_to = "nestedness") %>%
  select(-metric)
nichedf <- nichedf %>%
  pivot_wider(id_cols = "treatment",names_from = "group", values_from = "overlap") %>%
  rename("pollOverlap" = pollinators,
         "plantOverlap" = plants)

colnames(Total_niche)[1] <- "overlapNet"
rownames(Total_niche) <- c("pollinators", "plants")
Total_niche <- Total_niche %>%
  tibble::rownames_to_column("group")

# calculate total pollinator visitation per block  
visitation <- vector(mode ="logical", length = length(plist)) 
for(i in seq_along(plist)){
   this.mat <- as.matrix(plist[[i]])
   visitation[i] <- sum(this.mat)
 }
visitation <- data.frame( 
  treatment = names(plist), 
  total_visits = visitation )

# calculate species richness
richness <- vector(mode ="numeric", length =length(plist)) 
for (i in seq_along(plist)){
  this.logMat <- colnames(plist[[i]])
  richness[i] <- length(unique(this.logMat))
}
richness <- data.frame( 
  treatment = names(plist), 
  richness = richness )

# Join all characteristics toegther into df
Net_GroupDF <- nichedf %>%
  inner_join(richness, by = "treatment" ) %>%
  inner_join(visitation, by = "treatment" ) %>%
  inner_join(nestednessdf, by = "treatment") %>%
  inner_join(Hdf, by = "treatment", relationship = "many-to-many") %>%
  inner_join(Total_niche, by = "group", relationship = "many-to-one")

  
# combine all data 
seedsGrouped_wide <- seedsGrouped %>%
  pivot_wider(names_from = sp, values_from = ratio_seedsPerPlant) %>%
  inner_join(Net_GroupDF, by = "treatment")

# pivot df wider for tidyness 
spNames <- unique(seeds$sp)

WIDEfocalSeeds <- seedsGrouped_wide %>%
   pivot_longer(cols = all_of(spNames), names_to = "sp", values_to = "seedOutput") %>%
   rowwise() %>%
   mutate(AVoverlap = mean(c(pollOverlap,plantOverlap))) %>%
   group_by(genSpec) %>%
   mutate(meanH2 = mean(H2)) %>%
  select(-group)

WIDEfocalSeeds1 <- WIDEfocalSeeds

WIDEfocalSeeds1$genSpec <- gsub("genOnly", "Generalists", WIDEfocalSeeds1$genSpec)
WIDEfocalSeeds1$genSpec <- gsub("specOnly", "Specialists", WIDEfocalSeeds1$genSpec)
WIDEfocalSeeds1$genSpec <- gsub("genSpec1", "Mix1", WIDEfocalSeeds1$genSpec)
WIDEfocalSeeds1$genSpec <- gsub("genSpec3", "Mix2", WIDEfocalSeeds1$genSpec)
WIDEfocalSeeds1$genSpec <- gsub("genSpec4", "Mix3", WIDEfocalSeeds1$genSpec)
WIDEfocalSeeds1$Overlap <- WIDEfocalSeeds1$AVoverlap
WIDEfocalSeeds1$SeedOutput <- WIDEfocalSeeds1$seedOutput
WIDEfocalSeeds1$sp <- gsub("CLAAMO", "Clarkia amoena", WIDEfocalSeeds1$sp)
WIDEfocalSeeds1$sp <- gsub("CLAPUR", "Clarkia purpurea", WIDEfocalSeeds1$sp)
WIDEfocalSeeds1$sp <- gsub("MADELE", "Madia elegans", WIDEfocalSeeds1$sp)
WIDEfocalSeeds1$sp <- gsub("MICGRA", "Microsteris gracilis", WIDEfocalSeeds1$sp)


```

Visualise Whole Networks 

```{r}
# visualise network as heatmap
library(RColorBrewer)
cols <- hcl.colors(15, "Hawaii")
cols2 <- hcl.colors(15, "Red-Yellow", rev = TRUE)

names1 <- names(clist)

# heat map per treatment 
lapply(seq_along(clist), function(i) {
  par(mar = rep(0,4), oma = rep(0,4))
  visweb(clist[[i]],
          square = "defined", box.col = "black", def.col = cols2, labsize = 0.8, text = "interaction", textcol = "black", NA.col = "white")

 # title(main = names1[i], cex = 0.2, vjust = +10)
})

# make overlap models

lapply(seq_along(clist), function(i) {
  png(paste0(names(clist[i]),"webs.png"), units='in', width=4, height=4, res=300)
  plotweb(clist[[i]],
          empty = TRUE,
          higher_color = cols,
          lower_color = cols,
          text_size = 0.5,
          srt = 90)

 # title(main = names1[i], cex = 0.5)
  dev.off()
})




```

Plot biplots 

```{r}
library(ggbiplot)

pltpc <- function(pca, pc=c(1,2), group=NA,group2=NA, legInset=c(-.08,-.1), pcex=1.5){
#pcs <- prcomp(dat)
  nvar <- dim(pca$x)[2]
  if (any(!is.na(group))){
    gr = factor(group)
  } else {gr = factor(1)}
  if (any(!is.na(group2))){
    gr2 = factor(group2)
  } else {gr2 = factor(1)}
  groupcol <- c("#F4E7C5", "#678096", "#ACC2CF", "#979461", "#CD5733")
    #hcl.colors(length(levels(gr2)), "Plasma", alpha=0.9)
  loadings12 = c(sort(pca$rotation[,pc[1]]), 
                 sort(pca$rotation[,pc[2]]))
  #[c(1:5, (nvar-4):nvar)]
  nmloads = unique(names(loadings12))
  
  imp = summary(pca)$importance["Proportion of Variance",pc]*100
  xlab = paste0('PC',pc[1],' (',imp[1],'% explained)')
  ylab = paste0('PC',pc[2],' (',imp[2],'% explained)')
  
  plot(pca$x[,pc[1]], pca$x[,pc[2]], bg=groupcol[gr], col = groupcol[gr], pch=21, cex=pcex, xlab=xlab,
       ylab=ylab)
  for (s in nmloads){
    x1 <- pca$rotation[s,pc[1]]#*10
    y1 <- pca$rotation[s,pc[2]]#*10
    arrows(0,0,x1,y1,0.1)
    text(x1*1.1,y1*1.1,s, cex=0.5, srt = 90)
  }
  legend("bottomright",legend=levels(gr),pch=19,col=groupcol,
         inset=legInset,xpd=NA,bty='n', cex = 0.8)
}

# PCA for group-level
PCdf <- WIDEfocalSeeds %>%
  ungroup() %>% 
  select(c("richness", "total_visits", "AVoverlap", "nestedness", "H2", "plantOverlap", "pollOverlap")) %>%
  rename(Richness = "richness", Visitation = "total_visits", AverageOverlap = "AVoverlap", PollinatorOverlap = "pollOverlap", PlantOverlap = "plantOverlap", Nestedness = "nestedness")

PCAG <- prcomp(PCdf, scale. = TRUE)
biplot(PCAG, var.axes = TRUE, cex = 1, xlim = c(-0.3,0.3), ylim = c(-0.3,0.3))
pltpc(PCAG, group=WIDEfocalSeeds$genSpec, legInset=c(0,0))

WIDEfocalSeedsPCA <- cbind(WIDEfocalSeeds, PCAG$x[,1:2])


# PCA for species level 
PCdfSP <- speciesDF %>%
  ungroup() %>%
  select(c("degree", "PSI", "specialization"))

PCAS <- prcomp(PCdfSP, scale. = TRUE)
biplot(PCAS)

pltpc(PCAS, group=speciesDF$sp, legInset=c(0,0))

speciesPCA <- cbind(speciesDF, PCAS$x[,1:2])


ggplot(WIDEfocalSeedsPCA) +
  geom_point(aes(x = PC1, y = seedOutput, color = sp))
ggplot(WIDEfocalSeedsPCA) +
  geom_point(aes(x = PC2, y = seedOutput, color = sp))

ggplot(speciesPCA) +
  geom_point(aes(x = PC2, y = PC1, color = genSpec))

```
```{r}
WIDEfocalSeeds1 <- WIDEfocalSeeds1 %>% 
  group_by(sp) %>%
  mutate(fcseeds = mean(seedOutput)) %>%
  group_by(genSpec) %>%
  mutate(meanRICH = mean(richness), meanVIS = mean(total_visits))



```


