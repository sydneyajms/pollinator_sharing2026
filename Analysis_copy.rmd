...
smallSeeds1 <- smallSeeds %>%
  ...
  group_by(genSpec, sp, group) %>%
  summarise(AvOverlap = mean(overlap))

sort(smallSeeds1$AvOverlap)
...
test <- focalSeeds %>%
  group_by(genSpec) %>%
  summarise(meanrich = mean(MICGRA))
```
...
smallSeeds1 %>% filter(group=="pollinators") %>%
  droplevels()%>%
  ggplot(aes(x = AvOverlap, y = ratio_seedsPerPlant, color = genSpec)) +
  geom_boxplot() +
  facet_wrap(~sp)
...
ggplot(smallSeeds1) +
  geom_boxplot(aes(x = AvOverlap, y = ratio_seedsPerPlant, color = genSpec)) +
  facet_wrap(~sp)
