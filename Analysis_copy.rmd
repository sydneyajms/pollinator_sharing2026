...
ggplot(WIDEfocalSeeds) +
   geom_boxplot(aes(x = sp, y = seedOutput)) +
   facet_wrap(~genSpec) +
   labs(y ="Seed output", x = "Focal Species", title = "Reproductive Output: Treatment")

lm2 <- lm(WIDEfocalSeeds$richness~WIDEfocalSeeds$AVoverlap)
lm3 <- lm(WIDEfocalSeeds$total_visits~WIDEfocalSeeds$AVoverlap)
  
ggplot(WIDEfocalSeeds, aes(x = AVoverlap)) + 
  geom_point(aes(y = richness, color = "Richness")) + 
  geom_point(aes(y = total_visits, color = "Visitation")) + 
  scale_color_hue() +
  geom_abline(intercept = coef(lm2)[1], slope = coef(lm2)[2], color = "red") +
  geom_abline(intercept = coef(lm3)[1], slope = coef(lm3)[2], color = "lightblue") +
  labs(x = "Overlap", y = "Value")
