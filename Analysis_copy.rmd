...
seedsGrouped <- seeds %>%
  ...
  distinct(BP, genSpec, sp, ratio_seedsPerPlant, ratio_poll_impact) # remove unneccesary collumns
...
names(Total_niche)[1] <- "overlapNet"
...
####### Need to make BP for pollinator data as well 


# Join all characteristics toegther into df
Whole_df <- nichedf %>%
  inner_join(richness, by = c("treatment", relationship = "many-to-one" ) %>%
  inner_join(visitation, by = "treatment", relationship = "many-to-one" ) %>%
  inner_join(nestedness, by = "treatment", relationship = "many-to-one") %>%
  inner_join(Hdf, by = c("treatment", "group"), relationship = "many-to-many") %>%
  inner_join(Total_niche, by = "group", relationship = "many-to-many") %>%
  mutate(diffOverlap = overlapNet - overlap) %>%
  select(-c(overlapNet))
